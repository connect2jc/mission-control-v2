...
export const getStats = query({
  args: {},
  handler: async (ctx) => {
    const allDrafts = await ctx.db.query("linkedin_drafts").collect();
    const allFeedback = await ctx.db.query("linkedin_feedback").collect();
...
export const createDraft = mutation({
  args: {
    content: v.string(),
    contentType: v.optional(v.union(v.literal("post"), v.literal("article"), v.literal("comment"))),
    pillar: v.optional(v.string()),
    suggestedTime: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    return await ctx.db.insert("linkedin_drafts", {
...
export const submitFeedback = mutation({
  ...
  handler: async (ctx, args) => {
    const feedbackId = await ctx.db.insert("linkedin_feedback", {
